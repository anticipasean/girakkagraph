...
//    Option<C> getOrElseGet(ID id, Supplier<C> alternative);
    ...
    <R> Option<R> getAndMatch(ID id, Function<MatchClause<C>, R> patternMap);

    <R> Ent<ID, R> mapWithPattern(Pattern<C, R> patternFunc);
    ...
    default <T, U> Attribute<T> deriveAttributeFrom(Class<U> typeToken,
                                                    URI uri,
                                                    T value) {
        return PatternMatching.forObject(value)
