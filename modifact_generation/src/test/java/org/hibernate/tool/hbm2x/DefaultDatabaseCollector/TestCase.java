 ...
 *
 * <p>Contributor: Red Hat, Inc. - initial API and implementation
 * ****************************************************************************
 */
...
public class TestCase {
  ...
  public void testReadOnlySpecificSchema() {
    ...
    List<Table> tables =
        getTables(
            MetadataDescriptorFactory.createReverseEngineeringDescriptor(res, null)
                .createMetadata());
  ...
  public void testQuotedNamesAndDefaultDatabaseCollector() {
    ...
    DatabaseReader reader =
        DatabaseReader.create(properties, new DefaultStrategy(), realMetaData, serviceRegistry);
    RevengMetadataCollector dc = new RevengMetadataCollector();
    reader.readDatabaseSchema(dc);
    String defaultCatalog = properties.getProperty(AvailableSettings.DEFAULT_CATALOG);
    Assert.assertNotNull(
        "The table should be found",
        getTable(dc, realMetaData, defaultCatalog, "cat.cat", "cat.child"));
    Assert.assertNotNull(
        "The table should be found",
        getTable(dc, realMetaData, defaultCatalog, "cat.cat", "cat.master"));
    Assert.assertNull(
        "Quoted names should not return the table",
        getTable(
            dc, realMetaData, defaultCatalog, doubleQuote("cat.cat"), doubleQuote("cat.child")));
    Assert.assertNull(
        "Quoted names should not return the table",
        getTable(
            dc, realMetaData, defaultCatalog, doubleQuote("cat.cat"), doubleQuote("cat.master")));
    Assert.assertEquals(
        "Foreign key 'masterref' was filtered!", 1, dc.getOneToManyCandidates().size());
  }
