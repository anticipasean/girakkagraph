...
public interface RevengStrategy {
  ...
  /**
   * Close any resources this strategy might have used. Called after reverse engineering has been
   * completed.
   */
  void close();
   ...
   * @param generatedIdentifier true if for a column used in an identifier that is not "assigned",
   *     false otherwise.
   * @return The Preferred hibernate type name.
   */
  String columnToHibernateTypeName(
   ...
   * @return the identifier strategy name wanted for a specific table, null if use what the database
   *     metadata can tell.
   */
  ...
  boolean excludeForeignKeyAsCollection(
      String keyname,
  ...
  boolean excludeForeignKeyAsManytoOne(
      String keyname,
      TableIdentifier fromTable,
      List<?> fromColumns,
      TableIdentifier referencedTable,
  ...
  boolean isForeignKeyCollectionLazy(
      String name,
   ...
   *
   * @param fromColumns list of Column instances on the fromTable. Only col.getName() should be
   *     assumed to be correct
   * @param referencedColumns list of Column instances on the referenced Table. Only col.getName()
   *     should be assumed to be correct
   * @param uniqueReference true if there is no other references to the same table
   * @return
   */
  String foreignKeyToCollectionName(
      String keyname,
      TableIdentifier fromTable,
      List<?> fromColumns,
      TableIdentifier referencedTable,
   ...
   */
  String foreignKeyToInverseEntityName(
      String keyname,
      TableIdentifier fromTable,
      List<?> fromColumns,
      TableIdentifier referencedTable,
