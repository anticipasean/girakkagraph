...
/**
 * Interface for fetching metadata from databases. The dialect is configured with a
 * ConnectionProvider but is not required to actually use any connections.
 *
 * <p>The metadata methods all returns Iterator and allows for more efficient and partial reads for
 * those databases that has "flakey" JDBC metadata implementions.
 *
...
public interface RevengDialect {
   ...
   */
  void configure(ConnectionProvider connectionProvider);
   ...
   * @return iterator with map elements that has "TABLE_NAME", "TABLE_SCHEMA", "TABLE_CAT",
   *     "TABLE_TYPE" keys.
   */
   ...
   * @param table name or null
   * @return iterator with map elements that has "TABLE_NAME", "TABLE_SCHEMA", "TABLE_CAT",
   *     "INDEX_NAME", "COLUMN_NAME", "NON_UNIQUE", "TYPE" keys.
   */
   ...
   * @return iterator with map elements that has "TABLE_NAME", "TABLE_SCHEMA", "TABLE_CAT",
   *     "DATA_TYPE", "TYPE_NAME", "COLUMN_NAME", "NULLABLE", "COLUMN_SIZE", "DECIMAL_DIGITS"
   */
  Iterator<Map<String, Object>> getColumns(
   ...
   * @param table name or null
   * @return iterator with map elements that has "TABLE_NAME", "TABLE_SCHEMA", "TABLE_CAT",
   *     "FKTABLE_CAT", "FKTABLE_SCHEM", "FKTABLE_NAME", "FK_NAME", "KEY_SEQ"
   */
   ...
   * @param name
   * @return iterator with map elements that has "TABLE_NAME", "TABLE_SCHEMA", "TABLE_CAT",
   *     "HIBERNATE_STRATEGY" (null if no possible to determine strategy, otherwise return hibernate
   *     identifier strategy name/classname)
   */
  Iterator<Map<String, Object>> getSuggestedPrimaryKeyStrategyName(
