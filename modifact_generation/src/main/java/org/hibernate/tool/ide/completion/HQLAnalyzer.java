...
public class HQLAnalyzer {
  ...
  private static final String[] hqlKeywords = {
    "between",
    "class",
    "delete",
    "desc",
  ...
  /**
   * built-in function names. Various normal builtin functions in SQL/HQL. Maybe sShould try and do
   * this dynamically based on dialect or sqlfunctionregistry
   */
  ...
  /**
   * Returns true if the position is at a location where an entityname makes sense. e.g. "from Pr|
   * where x"
   *
   * @param query
  ...
  public boolean shouldShowEntityNames(char[] chars, int cursorPosition) {
    ...
    while ((tokenId = lexer.nextTokenId()) != HqlSqlTokenTypes.EOF) {
      if ((tokenId == HqlSqlTokenTypes.FROM
        ...
        show = true;
      } else if (tokenId != HqlSqlTokenTypes.DOT
          && tokenId != HqlSqlTokenTypes.AS
          && tokenId != HqlSqlTokenTypes.COMMA
          && tokenId != HqlSqlTokenTypes.IDENT
          && tokenId != HqlSqlTokenTypes.WS) {
        show = false;
      }
    }
    return show;
  ...
  public List<SubQuery> getVisibleSubQueries(char[] chars, int position) {
    ...
    for (Iterator<SubQuery> iter = sqList.subQueries.iterator(); iter.hasNext(); ) {
      ...
      if (sqList.caretDepth >= sq.depth
          && (sq.startOffset <= position || sq.endOffset >= position)) {
        visible.add(sq);
      }
    }
    return visible;
    ...
    while ((numericId = syntax.nextTokenId()) != HqlSqlTokenTypes.EOF) {
      ...
      if (numericId == HqlSqlTokenTypes.OPEN) {
        ...
        if (currentDepthQuery != null && currentDepthQuery.depth == depth) {
          ...
          currentDepthQuery.tokenText.add(
              String.valueOf(query, syntax.getTokenOffset(), syntax.getTokenLength()));
