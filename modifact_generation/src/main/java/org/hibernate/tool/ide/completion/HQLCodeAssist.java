...
public class HQLCodeAssist implements IHQLCodeAssist {
  ...
  private final ConfigurationCompletion completion;
  ...
  public void codeComplete(String query, int position, IHQLCompletionRequestor collector) {
    ...
    try {
      ...
      if (showEntityNames) {
        ...
        List<EntityNameReference> visible =
            new HQLAnalyzer().getVisibleEntityNames(query.toCharArray(), position);
        ...
        if (dotIndex == -1) {
          ...
          for (Iterator<EntityNameReference> iter = visible.iterator(); iter.hasNext(); ) {
            ...
            if (alias.startsWith(prefix)) {
              HQLCompletionProposal completionProposal =
                  new HQLCompletionProposal(HQLCompletionProposal.ALIAS_REF, position);
              ...
              if (hasMetadata()) {
                String importedName = metadata.getImports().get(qt.getEntityName());
