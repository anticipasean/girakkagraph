...
class ForeignKeyBinder extends AbstractBinder {
  ...
  void bindOutgoing(
    ...
    if (excludeForeignKeyAsManyToOne(foreignKey)) {
      // TODO: if many-to-one is excluded should the column be marked as processed so it won't show
      // up at all ?
  ...
  private void addOneToOne(
      ForeignKey foreignKey, PersistentClass rc, Set<Column> processedColumns, boolean outgoing) {
    Table table = outgoing ? foreignKey.getReferencedTable() : foreignKey.getTable();
    Property property =
        oneToOneBinder.bind(rc, table, foreignKey, processedColumns, outgoing, !outgoing);
    rc.addProperty(property);
  }
