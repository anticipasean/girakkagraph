...
class OneToManyBinder extends AbstractBinder {
  ...
  Property bind(PersistentClass rc, ForeignKey foreignKey) {
    ...
    return collectionPropertyBinder.bind(
  ...
  private KeyValue createKeyValue(
      Table collectionTable, ForeignKey foreignKey, KeyValue referencedKeyValue) {
    SimpleValue keyValue =
        new DependantValue(getMetadataBuildingContext(), collectionTable, referencedKeyValue);
  ...
  private void bindManyToMany(
      PersistentClass pc, ForeignKey fromForeignKey, Collection collection) {
    ForeignKey toForeignKey = getToForeignKey(fromForeignKey);
    bindCollection(pc, fromForeignKey, toForeignKey, collection);
    ManyToOne manyToOne =
        new ManyToOne(getMetadataBuildingContext(), collection.getCollectionTable());
  ...
  private void bindOneToMany(PersistentClass pc, ForeignKey fromForeignKey, Collection collection) {
    ...
    getMetadataCollector()
        .addSecondPass(new CollectionSecondPass(getMetadataBuildingContext(), collection));
  ...
  private String getFullRolePath(
      PersistentClass pc, ForeignKey fromForeignKey, ForeignKey toForeignKey) {
