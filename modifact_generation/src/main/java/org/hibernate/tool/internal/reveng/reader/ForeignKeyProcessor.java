...
public class ForeignKeyProcessor {
  ...
  public static ForeignKeyProcessor create(
    ...
    return new ForeignKeyProcessor(
        metaDataDialect, revengStrategy, defaultCatalog, defaultSchema, revengMetadataCollector);
  }
  ...
  /**
   * If catalog is equal to defaultCatalog then we return null so it will be null in the generated
   * code.
   */
  ...
  /**
   * If catalog is equal to defaultSchema then we return null so it will be null in the generated
   * code.
   */
  ...
  private static boolean equalTable(
      Table table1, Table table2, String defaultSchema, String defaultCatalog) {
  ...
  public ForeignKeysInfo processForeignKeys(Table referencedTable) {
    ...
    try {
      ...
      exportedKeyIterator =
          metaDataDialect.getExportedKeys(
      ...
      try {
        while (exportedKeyIterator.hasNext()) {
          ...
          String fkCatalog =
              getCatalogForModel((String) exportedKeyRs.get("FKTABLE_CAT"), defaultCatalog);
          String fkSchema =
              getSchemaForModel((String) exportedKeyRs.get("FKTABLE_SCHEM"), defaultSchema);
      ...
      log.warn(
          "Exception while reading foreign keys for "
              + referencedTable
              + " ["
              + se.toString()
              + "]",
    ...
    List<ForeignKey> userForeignKeys =
        revengStrategy.getForeignKeys(
    ...
    if (userForeignKeys != null) {
      ...
      while (iterator.hasNext()) {
        ...
        if (!equalTable(
            referencedTable, element.getReferencedTable(), defaultSchema, defaultCatalog)) {
          log.debug(
              "Referenced table "
                  + element.getReferencedTable().getName()
                  + " is not "
                  + referencedTable
                  + ". Ignoring userdefined foreign key "
                  + element);
        ...
        if (deptable != null) { // foreign key already defined!?
          throw new MappingException(
              "Foreign key " + userfkName + " already defined in the database!");
        }
    ...
    return new ForeignKeysInfo(
        referencedTable, dependentTables, dependentColumns, referencedColumns);
  }

  private Table getTable(String catalog, String schema, String name) {
    return revengMetadataCollector.getTable(
        TableIdentifier.create(quote(catalog), quote(schema), quote(name)));
  }
