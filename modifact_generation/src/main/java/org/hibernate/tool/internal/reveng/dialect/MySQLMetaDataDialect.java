...
public class MySQLMetaDataDialect extends JDBCMetaDataDialect {
  ...
  /**
   * Based on info from http://dev.mysql.com/doc/refman/5.0/en/show-table-status.html Should work on
   * pre-mysql 5 too since it uses the "old" SHOW TABLE command instead of SELECT from infotable.
   */
  ...
  public Iterator<Map<String, Object>> getColumns(
      String xcatalog, String xschema, String xtable, String xcolumn) {
    // MySql JDBC Driver doesn't like 'null' values for the table and column search patterns, use
    // '%' instead
    return super.getColumns(
        xcatalog, xschema, xtable != null ? xtable : "%", xcolumn != null ? xcolumn : "%");
  }
}
