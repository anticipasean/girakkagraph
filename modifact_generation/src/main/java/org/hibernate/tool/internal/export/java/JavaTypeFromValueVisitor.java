...
public class JavaTypeFromValueVisitor extends DefaultValueVisitor {
  ...
  private final boolean preferRawTypeNames = true;

  public JavaTypeFromValueVisitor() {
    super(true);
  }

  // special handling for Map's to avoid initialization of comparators that depends on the
  // keys/values which might not be generated yet.
  ...
  // special handling for Set's to avoid initialization of comparators that depends on the
  // keys/values which might not be generated yet.
  ...
  protected Object handle(Value o) {
    ...
    if (preferRawTypeNames && value.isSimpleValue()) {
      // this logic make us use the raw typename if it is something else than an Hibernate type. So,
      // if user wrote long we will use long...if he meant to have a Long then he should use the
