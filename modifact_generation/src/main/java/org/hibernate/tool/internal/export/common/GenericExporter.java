...
public class GenericExporter extends AbstractExporter {
        ...
        new ModelIterator() {
          void process(GenericExporter ge) {
            TemplateProducer producer =
                new TemplateProducer(ge.getTemplateHelper(), ge.getArtifactCollector());
            producer.produce(
    ...
    modelIterators.put(
        ...
        new ModelIterator() {
          void process(GenericExporter ge) {
            Iterator<?> iterator =
                ge.getCfg2JavaTool()
                    .getPOJOIterator(ge.getMetadata().getEntityBindings().iterator());
  ...
  protected String resolveFilename(POJOClass element) {
    String filename =
        StringHelper.replace(getFilePattern(), "{class-name}", getClassNameForFile(element));
  ...
  abstract static class ModelIterator {
