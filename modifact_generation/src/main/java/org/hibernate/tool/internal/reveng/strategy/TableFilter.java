...
/**
 * A tablefilter that can tell if a TableIdentifier is included or excluded. Note that all matching
 * is case sensitive since many db's are.
 *
 * @author max
 */
  ...
  private boolean isRelevantFor(TableIdentifier identifier) {
    if (catalogMatcher.match(identifier.getCatalog())) {
      if (schemaMatcher.match(identifier.getSchema())) {
        return nameMatcher.match(identifier.getName());
      }
    }
    return false;
  }
    ...
    boolean match(String matchEnum) {
      switch (mode) {
        case ANY:
          return true;
        case EQUALS:
          return this.value.equals(matchEnum);
        case ENDSWITH:
          return matchEnum.endsWith(this.value);
        case STARTSWITH:
          return matchEnum.startsWith(this.value);
        case SUBSTRING:
          return matchEnum.indexOf(this.value) >= 0;
        default:
