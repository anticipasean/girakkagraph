...
/**
 * Helper and wrapper for a Template engine (currently only FreeMarker). Exposes only the essential
 * functions to avoid too much coupling else where.
 *
 * @author max
 */
public class TemplateHelper {
  ...
  public void init(File outputDirectory, String[] templatePaths) {
    ...
    loaders.add(
        new ClassTemplateLoader(
            this.getClass(),
            "/")); // the template names are like pojo/Somewhere so have to be a rooted
    // classpathloader

    freeMarkerEngine.setTemplateLoader(
  ...
  public void ensureExistence(File destination) {
    ...
    if (dir.exists() && !dir.isDirectory()) {
      throw new RuntimeException(
          "The path: " + dir.getAbsolutePath() + " exists, but is not a directory");
    } // else make the directory and any non-existent parent directories
    else if (!dir.exists()) {
      if (!dir.mkdirs()) {
        if (dir.getName()
            .equals(
                ".")) { // Workaround that Linux/JVM apparently can't handle mkdirs of File's with
  ...
  public void setupContext() {
    ...
    getContext()
        .put(
            "ctx",
            getContext()); // TODO: I would like to remove this, but don't know another way to
    // actually get the list possible "root" keys for debugging.
  ...
  public void processTemplate(String templateName, Writer output, String rootContext) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Error while processing " + rootContext + " with template " + templateName, e);
    } catch (TemplateException te) {
      throw new RuntimeException(
          "Error while processing " + rootContext + " with template " + templateName, te);
    } catch (Exception e) {
      throw new RuntimeException(
          "Error while processing " + rootContext + " with template " + templateName, e);
    }
  ...
  /**
   * Check if the template exists. Tries to search with the templatePrefix first and then secondly
