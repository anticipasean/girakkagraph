    ...
    if (getProperties() != null) {
      ...
      while (iterator.hasNext()) {
        ...
        if (key.startsWith(ExporterSettings.PREFIX_KEY)) {
          getTemplateHelper()
              .removeFromContext(key.substring(ExporterSettings.PREFIX_KEY.length()), value);
        }
  ...
  protected void setupContext() {
    ...
    if (getProperties() != null) {
      ...
      while (iterator.hasNext()) {
        ...
        if (key.startsWith(ExporterSettings.PREFIX_KEY)) {
          getTemplateHelper()
              .putInContext(key.substring(ExporterSettings.PREFIX_KEY.length()), value);
          if (key.endsWith(".toolclass")) {
            try {
              ...
              throw new RuntimeException(
                  "Exception when instantiating tool " + element.getKey() + " with " + value, e);
            }
          }
        }
      }
