...
public class Cfg2HbmTool {
  ...
  /**
   * Remove any internal keys from the set, eg, any Keys that are prefixed by 'target_', {@link
   * PersistentIdentifierGenerator.IDENTIFIER_NORMALIZER} and return the filtered collection.
   *
   * @param properties
   * @return
   */
  public static Properties getFilteredIdentifierGeneratorProperties(
  ...
  private static boolean isDefaultSchema(String schema, Properties properties) {
    String defaultSchema = properties.getProperty(Environment.DEFAULT_SCHEMA);
    return defaultSchema == null ? schema == null : defaultSchema.equals(schema);
  }

  private static boolean isDefaultCatalog(String catalog, Properties properties) {
  ...
  public Set<?> getFilteredIdentifierGeneratorKeySet(Property property, Properties props) {
    return getFilteredIdentifierGeneratorProperties(
            this.getIdentifierGeneratorProperties(property), props)
        .keySet();
  }
  ...
  public boolean isOneToMany(Value value) {
    ...
    } else return value instanceof OneToMany;
  }
  ...
  public boolean needsDiscriminator(PersistentClass clazz) {
    ...
    return clazz instanceof Subclass
        && !(clazz instanceof UnionSubclass)
        && !(clazz instanceof JoinedSubclass);
  }
