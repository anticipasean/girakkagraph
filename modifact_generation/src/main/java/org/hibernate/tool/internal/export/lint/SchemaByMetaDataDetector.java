...
public class SchemaByMetaDataDetector extends RelationalModelDetector {
  ...
  public void initialize(Metadata metadata) {
    ...
    tableSelector = new TableSelectorStrategy(new DefaultStrategy());
    metadataDialect = RevengDialectFactory.createMetaDataDialect(dialect, properties);
    reader = DatabaseReader.create(properties, tableSelector, metadataDialect, serviceRegistry);
    ...
    while (iter.hasNext()) {
      ...
      if (!isSequence(key, sequences) && !isTable(key)) {
        collector.reportIssue(
            new Issue(
                "MISSING_ID_GENERATOR", Issue.HIGH_PRIORITY, "Missing sequence or table: " + key));
      }
    }
  }
  ...
  public void visit(Table table, Column col, IssueCollector pc) {
    ...
    Column dbColumn = currentDbTable.getColumn(new Column(col.getName()));
    ...
    if (dbColumn == null) {
      pc.reportIssue(
          new Issue(
              ...
              Issue.HIGH_PRIORITY,
              table(table) + " is missing column: " + col.getName()));
    } else {
      // TODO: this needs to be able to know if a type is truly compatible or not. Right now it
      // requires an exact match.
      // String sqlType = col.getSqlType( dialect, mapping );
      int dbTypeCode = dbColumn.getSqlTypeCode().intValue();
      int modelTypeCode = col.getSqlTypeCode(mapping);
        ...
        pc.reportIssue(
            new Issue(
                ...
                table(table)
                    ...
                    + col.getName()
                    + ", expected: "
                    + JdbcToHibernateTypeHelper.getJDBCTypeName(modelTypeCode)
                    + " but was "
                    + JdbcToHibernateTypeHelper.getJDBCTypeName(dbTypeCode)
                    + " in db"));
      }
    }
  }
  ...
  private void setSchemaSelection(Table table) {
    ...
    tableSelector.addSchemaSelection(
        createSchemaSelection(table.getCatalog(), table.getSchema(), table.getName()));
  }
  ...
  private Iterator<IdentifierGenerator> iterateGenerators() throws MappingException {
    ...
    Iterator<PersistentClass> persistentClassIterator =
    ...
    while (collectionIterator.hasNext()) {
      org.hibernate.mapping.Collection collection =
          (org.hibernate.mapping.Collection) collectionIterator.next();
  ...
  private SchemaSelection createSchemaSelection(
      String matchCatalog, String matchSchema, String matchTable) {
