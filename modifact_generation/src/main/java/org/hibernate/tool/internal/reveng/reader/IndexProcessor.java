...
public class IndexProcessor {
  ...
  public static void processIndices(
      RevengDialect metaDataDialect, String defaultSchema, String defaultCatalog, Table table) {

    Map<String, Index> indexes = new HashMap<String, Index>(); // indexname (String) -> Index
    Map<String, UniqueKey> uniquekeys =
        new HashMap<String, UniqueKey>(); // name (String) -> UniqueKey
    Map<Column, List<UniqueKey>> uniqueColumns =
        new HashMap<Column, List<UniqueKey>>(); // Column -> List<Index>

    Iterator<Map<String, Object>> indexIterator = null;
    try {
      Map<String, Object> indexRs = null;
      indexIterator =
          metaDataDialect.getIndexInfo(
      ...
      while (indexIterator.hasNext()) {
        ...
        if (columnName != null
            || indexName
                != null) { // both can be non-null with statistical indexs which we don't have any
          // use for.
      ...
      log.warn(
          "Exception while trying to get indexinfo on "
    ...
    Iterator<Entry<Column, List<UniqueKey>>> uniqueColumnIterator =
        uniqueColumns.entrySet().iterator();
