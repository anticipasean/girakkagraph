...
class VersionPropertyBinder extends AbstractBinder {
  ...
  void bind(Table table, RootClass rc, Set<Column> processed) {
    String optimisticLockColumnName =
        getRevengStrategy().getOptimisticLockColumnName(TableIdentifier.create(table));
  ...
  private void scanForAppropriateVersionColumn(Table table, RootClass rc, Set<Column> processed) {
  ...
  private void handleSpecifiedVersionColumn(
      Table table, String optimisticLockColumnName, RootClass rc, Set<Column> processed) {
    TableIdentifier identifier = TableIdentifier.create(table);
  ...
  private void bindVersionProperty(
      Table table, Column column, RootClass rc, Set<Column> processed) {
    TableIdentifier identifier = TableIdentifier.create(table);
    processed.add(column);
    String propertyName = getRevengStrategy().columnToPropertyName(identifier, column.getName());
    Property property =
        basicPropertyBinder.bind(BinderUtils.makeUnique(rc, propertyName), table, column);
