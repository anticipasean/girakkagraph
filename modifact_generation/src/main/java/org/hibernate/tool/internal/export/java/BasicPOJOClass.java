...
public abstract class BasicPOJOClass implements POJOClass, MetaAttributeConstants {
  ...
  static Map<String, DefaultInitializor> defaultInitializors =
      new HashMap<String, DefaultInitializor>();
  ...
  static {
    ...
    defaultInitializors.put(
        "java.util.SortedSet", new DefaultInitializor("java.util.TreeSet", false));
    defaultInitializors.put(
        "java.util.SortedMap", new DefaultInitializor("java.util.TreeMap", false));
  }
  ...
  /**
   * returns the properties that would be visible on this entity as a pojo. This does not return
   * *all* properties since hibernate has certain properties that are only relevant in context of
   * persistence.
   */
  ...
  // get the "opposite" collectionnae for a property. Currently a "hack" that just uses the same
  // naming algorithm as in reveng, will fail on more general models!
  ...
  private String internalGenerateArrayHashcode(
      Property property, String javaTypeName, String result, String thisName) {
    StringBuffer buf = new StringBuffer();
