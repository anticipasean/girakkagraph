...
public class DocFile {
  ...
  /**
   * Return a path-like reference to this file starting on the specified folder. The folder must be
   * a parent folder.
   *
  ...
  private String buildRefFromFolder(DocFolder folder) {
    ...
    if (index == -1) {
      throw new IllegalArgumentException(
          "The specified folder is not on this file's path: " + folder);
    }

    for (index++; index < folders.size(); index++) {
      DocFolder f = folders.get(index);
