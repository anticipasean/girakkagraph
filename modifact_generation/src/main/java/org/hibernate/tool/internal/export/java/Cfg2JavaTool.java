...
public class Cfg2JavaTool {
  ...
  /**
   * Convert string into something that can be rendered nicely into a javadoc comment. Prefix each
   * line with a star ('*').
   *
   * @param string
   */
  ...
  /**
   * Returns the typename for a property, using generics if this is a Set type and useGenerics is
   * set to true.
   */
    ...
    if (!StringHelper.isEmpty(overrideType)) {
      ...
      if (useGenerics && importType.indexOf("<") < 0) {
        if (p.getValue() instanceof Collection) {
          String decl =
              getGenericCollectionDeclaration((Collection) p.getValue(), true, importContext);
  ...
  private String getRawTypeName(
      Property p, boolean useGenerics, boolean preferRawTypeNames, ImportContext importContext) {
  ...
  public String getGenericCollectionDeclaration(
      Collection collection, boolean preferRawTypeNames, ImportContext importContext) {
    ...
    if (collection.isIndexed()) {
      ...
      if (!idxCol.isList()) {
        ...
        String indexType =
            importContext.importType(getJavaTypeName(idxElement, preferRawTypeNames));
   ...
   *     <p>TODO: handle this in a template ?
   */
  ...
  public String asNaturalIdParameterList(PersistentClass clazz) {
    ...
    while (fields.hasNext()) {
      ...
      if (field.isNaturalIdentifier()) {
        buf.append(getJavaTypeName(field, false)).append(" ").append(field.getName()).append(", ");
      }
    }
  ...
  public Iterator<POJOClass> getPOJOIterator(final Iterator<PersistentClass> persistentClasses) {
    return new Iterator<POJOClass>() {
      public POJOClass next() {
        return getPOJOClass(persistentClasses.next());
      }
