...
public class InstrumentationDetector extends EntityModelDetector {
  ...
  public void initialize(Metadata metadata) {
    ...
    if (bytecodeProvider instanceof org.hibernate.bytecode.internal.javassist.BytecodeProviderImpl
        || bytecodeProvider
            instanceof org.hibernate.bytecode.internal.bytebuddy.BytecodeProviderImpl) {
      enhanceEnabled = true;
    }
  }
      ...
      try {
        ...
        collector.reportIssue(
            new Issue(
                ...
                "lazy='true' set for '"
                    + clazz.getEntityName()
                    + "', but class has no default constructor."));
        return;
      }
      ...
      if (enhanceEnabled && !enhanced) {
        collector.reportIssue(
            new Issue(
                ...
                "'"
                    + clazz.getEntityName()
                    + "' has lazy='false', but its class '"
                    + mappedClass.getName()
                    + "' has not been instrumented with javaassist"));
        return;
      } else {
  ...
  protected void visitProperty(
      PersistentClass clazz, Property property, IssueCollector collector) {}
}
