...
public final class DocHelper {
  ...
  static final Comparator<POJOClass> POJOCLASS_COMPARATOR =
      new Comparator<POJOClass>() {
  ...
  /**
   * Map with Tables keyed by Schema FQN. The keys are Strings and the values are Lists of Tables
   */
  private final Map<String, List<Table>> tablesBySchema = new HashMap<String, List<Table>>();

  /**
   * Map with classes keyed by package name. PackageName is String key and values are List of
   * POJOClass
   */
  ...
  /**
   * Map where the keys are column names (tableFQN.column) and the values are lists with the Value
   * instances where those columns referenced.
   */
  private final Map<String, List<Value>> valuesByColumn = new HashMap<String, List<Value>>();

  /** Holds intances of Property keyed by Value objects. */
  private final Map<Value, List<Property>> propsByValue = new HashMap<Value, List<Property>>();

  /** List with all the tables. */
  private final List<Table> tables = new ArrayList<Table>();

  /**
   * Map that holds the Schema FQN for each Table. The keys are Table instances and the values are
   * Strings with the Schema FQN for that table.
   */
  ...
  /**
   * Return a Map with the tables keyed by Schema. The keys are the schema names and the values are
   * Lists of tables.
   *
  ...
  /**
   * Returns the qualified schema name for a table. The qualified schema name will include the
   * catalog name if one is specified.
   *
  ...
  public String getQualifiedSchemaName(Table table) {
    ...
    return tableSchemaNames.get(table);
  }
  ...
  /**
   * Method used in class.vm template to get the ComponentPOJO class corresponding to Property if
   * its of Type Component.
   *
  ...
  public List<Property> getSimpleProperties(POJOClass pojoClass) {
    ...
    if (pojoClass.hasIdentifierProperty()) properties.remove(pojoClass.getIdentifierProperty());
    // TODO: do we need to also remove component id properties?
    if (pojoClass.hasVersionProperty()) properties.remove(pojoClass.getVersionProperty());
    return properties;
  }
