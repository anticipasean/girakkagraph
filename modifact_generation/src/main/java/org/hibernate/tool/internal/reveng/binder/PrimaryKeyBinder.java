...
class PrimaryKeyBinder extends AbstractBinder {
  ...
  PrimaryKeyInfo bind(
    ...
    SimpleValue id =
        createKeyValue(
            rc, keyColumns, pki.suggestedStrategy, table, revengMetadataCollector, processed);
  ...
  private PrimaryKeyInfo createPrimaryKeyInfo(
      TableIdentifier tableIdentifier, List<Column> keyColumns) {
    ...
    if (keyColumns.size() == 1) {
      result.suggestedStrategy =
          RevengUtils.getTableIdentifierStrategyNameInRevengStrategy(
              getRevengStrategy(), tableIdentifier, getDefaultCatalog(), getDefaultSchema());
    }
    return result;
  }

  private String getTableIdentifierStrategyName(
      String suggestedStrategy, RevengMetadataCollector revengMetadataCollector, Table table) {
    if (suggestedStrategy == null) {
      suggestedStrategy =
          revengMetadataCollector.getSuggestedIdentifierStrategy(
  ...
  private String getIdPropertyName(TableIdentifier tableIdentifier, List<Column> keyColumns) {
    ...
    if (result == null) {
      if (keyColumns.size() > 1) {
        ...
        result =
            getRevengStrategy().columnToPropertyName(tableIdentifier, keyColumns.get(0).getName());
      }
    }
    return result;
  }

  private boolean isGeneratedId(List<Column> keyColumns, String tableIdentifierStrategyName) {
  ...
  private List<Column> getKeyColumns(Table table) {
    ...
    if (table.getPrimaryKey() != null) {
      ...
      LOGGER.log(
          Level.INFO,
          "No primary key found for " + table + ", using all properties as the identifier.");
  ...
  private SimpleValue handleColumnKey(
    ...
    processed.add(pkc);
    SimpleValue result =
        simpleValueBinder.bind(table, pkc, isGeneratedId(keyColumns, tableIdentifierStrategyName));
    result.setIdentifierGeneratorStrategy(tableIdentifierStrategyName);
    return result;
  }

  private SimpleValue handleCompositeKey(
      PersistentClass rc, Set<Column> processedColumns, List<Column> keyColumns) {
    ...
    addKeyColumns(
        result, rc.getTable(), getKeyColumns(rc.getTable(), keyColumns), processedColumns);
  ...
  private Property bindManyToOneProperty(
      Component pkc, ForeignKeyForColumns fkfc, Table table, Set<Column> processedColumns) {
    ForeignKey foreignKey = fkfc.key;
    Property property =
        manyToOneBinder.bind(
  ...
  private Property bindBasicProperty(
      Component pkc, Column column, Table table, Set<Column> processedColumns) {
    ...
    if (processedColumns.contains(column)) {
      ...
      result =
          basicPropertyBinder.bind(
              BinderUtils.makeUnique(pkc, getColumnToPropertyName(table, column)), table, column);
  ...
  private String getColumnToPropertyName(Table table, Column column) {
    return getRevengStrategy()
        .columnToPropertyName(TableIdentifier.create(table), column.getName());
  }

  private void addKeyColumns(
      Component pkc, Table table, List<?> list, Set<Column> processedColumns) {
    for (Iterator<?> iter = list.iterator(); iter.hasNext(); ) {
      ...
      if (element instanceof Column) {
        ...
        property =
            bindManyToOneProperty(pkc, (ForeignKeyForColumns) element, table, processedColumns);
      } else {
  ...
  private String getCompositeIdName(PersistentClass pc) {
    String compositeIdName =
        getRevengStrategy().tableToCompositeIdName(TableIdentifier.create(pc.getTable()));
