...
public class OverrideBinder {
  ...
  private static void bindSchemaSelections(
      ArrayList<Element> schemaSelections, OverrideRepository repository) {
  ...
  private static void bindTypeMappings(
      ArrayList<Element> typeMappings, OverrideRepository repository) {
  ...
  private static void bindTableFilters(
      ArrayList<Element> tableFilters, OverrideRepository repository) {
    for (Element element : tableFilters) {
      ...
      MultiValuedMap<String, SimpleMetaAttribute> map =
          MetaAttributeHelper.loadAndMergeMetaMap(
              element, new HashSetValuedHashMap<String, MetaAttributeHelper.SimpleMetaAttribute>());
  ...
  private static void bindPrimaryKey(Element element, Table table, OverrideRepository repository) {
    ...
    List<String> boundColumnNames =
        bindColumns(getChildElements(element, "key-column"), table, repository);
  ...
  private static List<String> bindColumns(
      ArrayList<Element> columns, Table table, OverrideRepository repository) {
    ...
    for (Element element : columns) {
      ...
      if (StringHelper.isNotEmpty(attributeValue)) {
        column.setSqlTypeCode(
            Integer.valueOf(JdbcToHibernateTypeHelper.getJDBCType(attributeValue)));
      }
      TableIdentifier tableIdentifier = TableIdentifier.create(table);
      if (table.getColumn(column) != null) {
        throw new MappingException(
            "Column " + column.getName() + " already exists in table " + tableIdentifier);
      }
      MultiValuedMap<String, SimpleMetaAttribute> map =
          MetaAttributeHelper.loadAndMergeMetaMap(
              element, new HashSetValuedHashMap<String, MetaAttributeHelper.SimpleMetaAttribute>());
      ...
      repository.setTypeNameForColumn(
          tableIdentifier, column.getName(), getAttribute(element, "type"));
      repository.setPropertyNameForColumn(
          tableIdentifier, column.getName(), getAttribute(element, "property"));
      ...
      if (element.hasAttribute("foreign-table")) {
        ...
        if (element.hasAttribute("foreign-column")) {
          ...
          throw new MappingException(
              "foreign-column is required when foreign-table is specified on " + column);
        }
        ForeignKey key =
            table.createForeignKey(null, localColumns, foreignTableName, null, foreignColumns);
        key.setReferencedTable(
            foreignTable); // only possible if foreignColumns is explicitly specified (workaround on
        // aligncolumns)
      }
    }
    return columnNames;
  ...
  private static void bindForeignKeys(
      ArrayList<Element> foreignKeys, Table table, OverrideRepository repository) {
      ...
      if (foreignTableName != null) {
        ...
        ForeignKey key =
            table.createForeignKey(
                constraintName, localColumns, foreignTableName, null, foreignColumns);
        key.setReferencedTable(
            foreignTable); // only possible if foreignColumns is explicitly specified (workaround on
        // aligncolumns)
      }
      if (StringHelper.isNotEmpty(constraintName)) {
        if (!validateFkAssociations(element)) {
          throw new IllegalArgumentException(
              "you can't mix <many-to-one/> or <set/> with <(inverse-)one-to-one/> ");
        }
  ...
  private static void bindOneToOne(
      Element element, String constraintName, OverrideRepository repository) {
  ...
  private static AssociationInfo extractAssociationInfo(Element manyToOne) {
    return RevengUtils.createAssociationInfo(
        ...
        manyToOne.hasAttribute("insert")
            ? Boolean.parseBoolean(manyToOne.getAttribute("insert"))
            : null,
        manyToOne.hasAttribute("update")
            ? Boolean.parseBoolean(manyToOne.getAttribute("update"))
            : null);
  }
  ...
  private static void bindMetaAttributes(
      Element element, Table table, OverrideRepository repository) {
    MultiValuedMap<String, SimpleMetaAttribute> map =
        MetaAttributeHelper.loadAndMergeMetaMap(
            element, new HashSetValuedHashMap<String, MetaAttributeHelper.SimpleMetaAttribute>());
    if (map != null && !map.isEmpty()) {
  ...
  private static void bindSchemaSelection(
      Element schemaSelectionElement, OverrideRepository repository) {
  ...
  private static void bindSqlType(Element sqlType, OverrideRepository repository) {
    int jdbcType = JdbcToHibernateTypeHelper.getJDBCType(getAttribute(sqlType, "jdbc-type"));
    SQLTypeMapping sqlTypeMapping = new SQLTypeMapping(jdbcType);
    sqlTypeMapping.setHibernateType(getHibernateType(sqlType));
    sqlTypeMapping.setLength(
        getInteger(getAttribute(sqlType, "length"), SQLTypeMapping.UNKNOWN_LENGTH));
    sqlTypeMapping.setPrecision(
        getInteger(getAttribute(sqlType, "precision"), SQLTypeMapping.UNKNOWN_PRECISION));
    sqlTypeMapping.setScale(
        getInteger(getAttribute(sqlType, "scale"), SQLTypeMapping.UNKNOWN_SCALE));
