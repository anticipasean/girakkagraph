...
/**
 * Evaluator which uses reflection to get path from an object. Uses expression visitor API to
  ...
  private static class LocalVisitor implements ExpressionVisitor<Object> {
    ...
    private static Object binaryCall(Call call, Object left, Object right) {
      Preconditions.checkArgument(
          call.operator().arity() == Operator.Arity.BINARY, "Expected binary call got %s", call);
      ...
      if (op == Operators.IN || op == Operators.NOT_IN) {
        Preconditions.checkArgument(
            right instanceof Iterable, "%s is not iterable", left.getClass());
        ...
        return op == Operators.IN
            ? stream.anyMatch(r -> Objects.equals(left, r))
            : stream.noneMatch(r -> Objects.equals(left, r));
      }
      ...
      if (op == StringOperators.MATCHES) {
        Preconditions.checkArgument(
            left instanceof CharSequence, "%s is not string (or CharSequence)", left);
      ...
      if (op == StringOperators.STARTS_WITH
          ...
          || op == StringOperators.CONTAINS) {
        Preconditions.checkArgument(
            left instanceof CharSequence, "%s is not string (or CharSequence)", left);
        Preconditions.checkArgument(
            right instanceof CharSequence, "%s is not string (or CharSequence)", right);

        if (op == StringOperators.CONTAINS) {
          return left.toString().contains(right.toString());
        }

        return op == StringOperators.STARTS_WITH
    ...
    public Object visit(Call call) {
      ...
      if (op == Operators.NOT) {
        Preconditions.checkArgument(
            args.size() == 1, "Size should be 1 for %s but was %s", op, args.size());
        ...
        throw new UnsupportedOperationException(
            String.format("Expected boolean for op %s but got %s", op, value.getClass().getName()));
      }
      ...
      if (op == IterableOperators.IS_EMPTY || op == IterableOperators.NOT_EMPTY) {
        ...
        Preconditions.checkArgument(
            value instanceof Iterable, "%s not iterable", value.getClass().getName());
      ...
      if (op.arity() == Operator.Arity.BINARY) {
        Preconditions.checkArgument(
            ...
            "Expected two arguments for %s got %s",
            call,
            call.arguments().size());
