...
public final class InMemory {
  ...
  /**
   * Converts criteria to a predicate. Note that filter must be set on criteria. For instance {@code
   * person.name.is("John")} is a valid filter but {@code person.name} is not.
   *
  ...
  public static <T> Predicate<T> toPredicate(Criterion<T> criterion) {
    ...
    Expression expression =
        Criterias.toQuery(criterion)
            .filter()
            .orElseThrow(() -> new IllegalArgumentException("Filter not set"));
