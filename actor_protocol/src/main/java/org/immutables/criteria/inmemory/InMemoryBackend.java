...
import org.immutables.criteria.backend.ImmutableWriteResult;
...
public class InMemoryBackend implements Backend {
  ...
  private Logger logger = LoggerFactory.getLogger(InMemoryBackend.class);
  ...
  public Session open(Class<?> entityType) {
    final Map<Object, Object> store =
        classToStore.computeIfAbsent(entityType, key -> new ConcurrentHashMap<>());
  ...
  private static class Session implements Backend.Session {
    ...
    private Publisher<?> query(StandardOperations.Select select) {
      ...
      if (query.hasAggregations()) {
        throw new UnsupportedOperationException(
            "Aggregations are not yet supported by " + InMemoryBackend.class.getSimpleName());
      }
      ...
      if (!query.collations().isEmpty()) {
        ...
        for (Collation collation : query.collations()) {
          Function<Object, Comparable<Object>> fn =
              obj -> (Comparable<Object>) pathExtractor.extract(collation.path(), obj);
          @SuppressWarnings("unchecked")
    ...
    private Publisher<WriteResult> update(StandardOperations.Update op) {
      ...
      Map<Object, Object> toUpdate =
          op.values().stream().collect(Collectors.toMap(idExtractor::extract, v -> v));

      if (op.upsert()) {
        return Flowable.fromCallable(
            () -> {
              store.putAll(toUpdate);
              return ImmutableWriteResult.builder()
                  .updatedCount(updatedCount.updateAndGet(operand -> operand + toUpdate.size()))
                  .build();
            });
      }
    ...
    private Publisher<WriteResult> insert(StandardOperations.Insert op) {
      //      logger.info("beginning insert op: " + op);
      if (op.values().isEmpty()) {
        return Flowable.just(WriteResult.empty());
      }
      ...
      final Map<Object, Object> toInsert =
          op.values().stream().collect(Collectors.toMap(idExtractor::extract, x -> x));
      toInsert.forEach(
          (k, v) -> {
            //            logger.info(String.format("storing: [ k: %s v: %s ]", k,v));
      ...
      //      logger.info("ending insert op");
      return Flowable.just(
          ImmutableWriteResult.builder().insertedCount(insertCounter.get()).build());
    }
  }
}
